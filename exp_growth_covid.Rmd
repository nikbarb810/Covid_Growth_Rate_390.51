...
int pairwisediff_cpp(CharacterVector arr1, CharacterVector arr2) {
  // types CharacterVector MUST be converted to c++ strings to achieve functionality of cpp lib funcs, like            // str.size()
  std::string str1 = Rcpp::as<std::string>(arr1);
  std::string str2 = Rcpp::as<std::string>(arr2);
...
// [[Rcpp::export]]
double k_estimate_cpp(List simulation) {
  ...
  for (int i = 0; i < (num_arrays - 1); ++i) {
    for (int j = (i + 1); j < num_arrays; ++j) {
      ...
      sum_diff += pairwisediff_cpp(simulation[i], simulation[j]);
    }
  }
  
  // Calculate average
  double k_val = sum_diff / Rf_choose(num_arrays, 2);
...
Below we can see a speed comparison between the 2 options, on a single random simulation that contains 500 strings
```{r}
...
print(r_sims_time) #676 seconds

#calc time in cpp
cpp_sims_time <- system.time(lapply(simulations,k_estimate_cpp))
print(cpp_sims_time) #6.7 seconds
...
time = system.time(lapply(simulations,tajimas_D))
time
